...
export const postStudent = async (req, res) => {
    ...
    //Se valida la integridad de lo datos
        if(!req.body.name || !req.body.major || !req.body.age || typeof req.body.age !== "number" || req.body.age < 0){
    ...
    const isNotUnique = studentsList.some((student)=> student.id === id);
 
    if(isNotUnique) return res.status(409)
        .json({error: 'El estudiante ya existe'})
